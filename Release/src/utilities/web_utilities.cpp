...
utility::secure_unique_wstring winrt_encryption::decrypt() const
{
    ...
    auto data = utility::make_unique<utility::secure_wstring, utility::secure_unique_ptr_deleter<utility::secure_wstring>>(
        reinterpret_cast<const utility::wstring::value_type*>(rawPlaintext), plaintext->Length / 2));
...
win32_encryption::win32_encryption(const utility::secure_wstring& data) : m_numCharacters(data.size())
{
...
utility::secure_unique_wstring win32_encryption::decrypt() const {
    ...
    auto result = utility::make_unique<utility::secure_wstring, utility::secure_unique_ptr_deleter<utility::secure_wstring>>(reinterpret_cast<const utility::secure_wstring::value_type*>(m_buffer.data()), m_buffer.size());
    auto& data = *result;
    if (!m_buffer.empty())
    {
